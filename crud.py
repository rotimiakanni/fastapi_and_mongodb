...
class CRUDService:
    ...
    @staticmethod
    def create_book(book_data: schema.BookCreate) -> dict:
        book_data = jsonable_encoder(book_data)
        book_document_data = books_collection.insert_one(book_data)
        book_id = book_document_data.inserted_id
        book_document = books_collection.find_one({"_id": ObjectId(book_id)})
        return serializer.book_serializer(book_document)
    
    @staticmethod
    def get_all_books(skip: int = 0, limit: int = 10) -> list:
    ...
    @staticmethod
    def update_book(book_id: str, book_update_in: schema.BookUpdate) -> Optional[dict]:
        book = books_collection.find_one({"_id": ObjectId(book_id)})
        if not book:
            return None
    ...
    def delete_book(book_id: str) -> Optional[dict]:
        book = books_collection.find_one_and_delete({"_id": ObjectId(book_id)})
        if book:
            return serializer.book_serializer(book)
        return None
...
class UserCRUDService:
    ...
    @staticmethod
    def create_user(user_data: schema.UserCreate, hashed_password: str) -> dict:
    ...
    @staticmethod
    def get_user_by_username(username: str) -> Optional[schema.UserInDB]:
    ...
    @staticmethod
    def get_user_by_username_with_hash(username: str) -> Optional[schema.UserInDB]:
        user = users_collection.find_one({"username": username})
        if user:
